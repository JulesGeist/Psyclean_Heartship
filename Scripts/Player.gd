...
var active = true
@onready var pick_length : float = 1
@onready var pick_timeout = $Input_Timeout.time_left
@onready var pocket : Node = get_node("../UI/Inventory/Pocket")
...
func _physics_process(_delta):
	...
	if active == true:
		...
		pick_timeout = $Input_Timeout.time_left
...
func attack_picker():
	...
	if trash_in_range.size() > 0:
		...
		# if trash is powerup, activate its action
		if closest_trash.is_in_group("powerup"):
			$PickupAnimation.global_position = closest_trash.global_position
			$PickupAnimation.visible = true
			$PickupAnimation.play("pickup_powerup")
			closest_trash.powerup_action(self)
			return
			
		# if trash has loot, add it to inv by path
		if closest_trash.loot != "nothing":
			var loot = closest_trash.loot
			pocket.spawn_item(loot)
		$PickupAnimation.global_position = closest_trash.global_position
		$PickupAnimation.visible = true
		$PickupAnimation.play("pickup_picker")
