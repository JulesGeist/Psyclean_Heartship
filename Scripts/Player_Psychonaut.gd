...
@onready var reload_timeout = $ReloadTimer.time_left
@onready var reload = $Range_Toy/CollisionShape2D/Toy.reload
signal ep_send
...
func _ready():
	...
	active = false
	reload_timer.set_wait_time(toy.reload)
...
func _physics_process(delta):
	...
	reload_timeout = $ReloadTimer.time_left
...
func give_ep(npc):
	...
	if toy.ep_over_time > 0.0:
		...
		for i in range(1, toy.ep_over_time) :
			dot_npc.empathy += 1
